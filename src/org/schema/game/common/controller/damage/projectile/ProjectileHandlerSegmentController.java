...
import thederpgamer.systemsplus.handler.SegmentPieceDamageHandler;

import javax.vecmath.Vector3f;
import java.util.List;

/**
 * Modified version of ProjectileHandlerSegmentController.
 *
...
public class ProjectileHandlerSegmentController extends ProjectileHandler{
    public class ShotHandler{
        ...
        private LongArrayList positionsHit = new LongArrayList();
        private ObjectArrayList<ElementInformation> typesHit = new ObjectArrayList<ElementInformation>();
        private ObjectArrayList<Segment> segmentsHit = new ObjectArrayList<Segment>();
        private ObjectOpenHashSet<Segment> segmentsHitSet = new ObjectOpenHashSet<Segment>();
        private IntArrayList infoIndexHit = new IntArrayList();
...
//						}
    ...
    private boolean processHitsUnshielded(ShotHandler shotHandler) {
        final int accumulated = shotHandler.positionsHit.size();
        ...
        if(accumulated > 1){
            ...
            for(int i = 0; i < accumulated; i++){
                final ElementInformation info = shotHandler.typesHit.get(i);
                final Segment currentSeg = shotHandler.segmentsHit.get(i);
                final int infoIndex = shotHandler.infoIndexHit.get(i);
                final short type = info.id;
                ...
                if(info.isArmor()){
                    ...
                    armorValueTotal += info.getArmorValue() + (info.getArmorValue() * ((float)armorBlockCount * VoidElementManager.ARMOR_THICKNESS_BONUS));
                    armorIntegrity += (float)currentSeg.getSegmentData().getHitpointsByte(infoIndex) * ElementKeyMap.MAX_HITPOINTS_INV; // div by 127
//					if(isOnServer()) {
...
//					}
            ...
            if(armorBlockCount > 0){
                ...
                shotHandler.dmg = shotHandler.hitSegController.getHpController().onHullDamage(
                        shotHandler.damager, damageOnBlock,
                        firstArmorId, shotHandler.damageDealerType);
...
//				}
        ...
        for(int i = 0; i < accumulated && shotHandler.dmg > 0; i++){
            ...
            if(rayCallbackTraverse.isDebug()) {
                System.err.println("HANDLING DAMAGE ON BLOCK: "+info+"; "+new SegmentPiece(currentSeg, infoIndex));
            }
...
//					}
    ...
    private void doDamageReduction(ShotHandler shotHandler, int armorBlockCount) {
        if(VoidElementManager.ARMOR_CALC_STYLE == ArmorDamageCalcStyle.EXPONENTIAL) {
            ...
            shotHandler.dmg = Math.max(0,
                    FastMath.pow(shotHandler.dmg, VoidElementManager.CANNON_ARMOR_EXPONENTIAL_INCOMING_EXPONENT) /
                            (FastMath.pow(shotHandler.totalArmorValue, VoidElementManager.CANNON_ARMOR_EXPONENTIAL_ARMOR_VALUE_TOTAL_EXPONENT) +
                                    FastMath.pow(shotHandler.dmg, VoidElementManager.CANNON_ARMOR_EXPONENTIAL_INCOMING_DAMAGE_ADDED_EXPONENT)));
        }else {
            shotHandler.dmg = Math.max(0, shotHandler.dmg - (VoidElementManager.CANNON_ARMOR_FLAT_DAMAGE_REDUCTION *  shotHandler.dmg));
            shotHandler.dmg = Math.max(0, shotHandler.dmg - Math.min(VoidElementManager.CANNON_ARMOR_THICKNESS_DAMAGE_REDUCTION_MAX, ((VoidElementManager.CANNON_ARMOR_THICKNESS_DAMAGE_REDUCTION * (float)armorBlockCount)) *  shotHandler.dmg));
        }
    }
    private float doDamageOnBlock(final short type, final ElementInformation info, final Segment currentSeg, final int infoIndex){
...
//		}
        ...
        final float damageOnBlock;

        if(shotHandler.blockIndex == 0) {

            //reduce damage according to armor hit on first hit
            final InterEffectSet defense = shotHandler.defenseEffectSetTmp;
            final InterEffectSet globalDefense = info.isArmor() ? shotHandler.defenseArmor : shotHandler.defenseBlock;
            assert(shotHandler.damager != null);
            assert(shotHandler.damageDealerType != null);
            ...
            final InterEffectSet attack = shotHandler.damager.getAttackEffectSet(shotHandler.weaponId, shotHandler.damageDealerType);
            final HitReceiverType hitReceiverType = info.isArmor() ? HitReceiverType.ARMOR : HitReceiverType.BLOCK;
        ...
        //INSERTED CODE
        final int damage = (int) SegmentPieceDamageHandler.handle(new SegmentPiece(currentSeg, infoIndex), Math.round(damageOnBlock));
        //
        ...
        if(currentSeg.getSegmentController() instanceof ManagedSegmentController<?> && (info.isInventory() || type == ElementKeyMap.CARGO_SPACE)){
            ...
            final long absIndex = currentSeg.getAbsoluteIndex(infoIndex);
        ...
        final short hitpointsBefByte = currentSeg.getSegmentData().getHitpointsByte(infoIndex);
        ...
        if(eSeg.isExtraAcidDamageOnDecoBlocks() && info.isDecorative()) {
            if(isOnServer()) {
                ...
                shotHandler.acidFormula.getAcidDamageSetting(
                        type,
                        damage,
                        extraDamageFromDecoHit,
                        extraDamageFromDecoHit,
                        shotHandler.totalArmorValue,
                        shotHandler.blockIndex,
                        shotHandler.projectileWidth,
                        shotHandler.penetrationDepth,
                        shotHandler.shotStatus,
                        shotHandler.acidSetting);
                ...
                eSeg.getAcidDamageManagerServer().inputDamage(
                        currentSeg.getAbsoluteIndex(infoIndex),
                        shotHandler.shootingDirRelative,
                        shotHandler.acidSetting.damage,
                        shotHandler.acidSetting.maxPropagation,
                        shotHandler.damager,
                        shotHandler.weaponId,
                        propagateOnInitallyKilled,
                        decoOnly
                );
            }else {
    ...
    public ProjectileHandleState handle(Damager damager, ProjectileController projectileController,
        ...
        final SegmentController c = seg.getSegmentController();
...
//			rayCallbackTraverse.setDebug(isOnServer());
            ...
            ((ModifiedDynamicsWorld) projectileController.getCurrentPhysics().getDynamicsWorld()).rayTest(posBeforeUpdate, posAfterUpdate, rayCallbackTraverse);
            ...
            if(shotHandler.getResult() == ProjectileHandleState.PROJECTILE_HIT_STOP || shotHandler.getResult() == ProjectileHandleState.PROJECTILE_HIT_CONTINUE ||
                    shotHandler.getResult() == ProjectileHandleState.PROJECTILE_HIT_STOP_INVULNERABLE) {
    ...
    public ProjectileHandleState handleBefore(Damager damager,
        ...
        shotHandler.acidFormula = AcidFormulaType.values()[particles.getAcidFormulaIndex(particleIndex)].formula;
...
//		}
        ...
        if(c instanceof ManagedSegmentController<?>){
            ...
            if(man instanceof ShieldContainerInterface){
                ShieldContainerInterface sh = (ShieldContainerInterface) man;



                boolean localShieldOn = (c.isUsingLocalShields() &&
                        (sh.getShieldAddOn().isUsingLocalShieldsAtLeastOneActive() || c.railController.isDockedAndExecuted()));
                boolean oldShieldsOn = (sh.getShieldAddOn().getShields() > 0 || c.railController.isDockedAndExecuted());
                if( localShieldOn || oldShieldsOn){

                    final float bef = shotHandler.dmg;
                    final InterEffectSet shieldDefense = shotHandler.defenseEffectSetTmp;
                    ...
                    try {
                        shotHandler.dmg = (float) sh.getShieldAddOn().handleShieldHit(
                                damager,
                                shieldDefense,
                                rayCallbackInitial.hitPointWorld,
                                projectileController.getSectorId(),
                                DamageDealerType.PROJECTILE,
                                HitType.WEAPON,
                                shotHandler.dmg, shotHandler.weaponId
                        );
    ...
    @Override
    public void afterHandleAlways(Damager damager,
                                  ProjectileController projectileController,
                                  Vector3f posBeforeUpdate, Vector3f posAfterUpdate,
                                  ProjectileParticleContainer particles, int particleIndex,
                                  CubeRayCastResult rayCallbackInitial) {
